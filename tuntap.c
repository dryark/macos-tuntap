...
static PyGetSetDef pytun_tuntap_prop[] = {
    ...
    { "addr", NULL, pytun_tuntap_set_addr, NULL, NULL },
    { "dstaddr", NULL, NULL, NULL, NULL },
    { "hwaddr", NULL, NULL, NULL, NULL },
    { "netmask", NULL, NULL, NULL, NULL },
...
PyDoc_STRVAR(pytun_tuntap_close_doc, "close() -> None.\nClose the device.");
...
PyDoc_STRVAR( pytun_tuntap_up_doc, "up() -> None.\nBring up the device." );
...
PyDoc_STRVAR( pytun_tuntap_down_doc, "down() -> None.\nBring down the device." );
...
PyDoc_STRVAR(pytun_tuntap_read_doc, "read(size) -> read at most size bytes, returned as a string.");
...
PyDoc_STRVAR(pytun_tuntap_write_doc, "write(str) -> number of bytes written.\nWrite str to device.");

static PyObject *pytun_tuntap_fileno(PyObject *self) {
    return PyLong_FromLong(((pytun_tuntap_t *) self)->fd);
}

PyDoc_STRVAR(pytun_tuntap_fileno_doc, "fileno() -> integer \"file descriptor\".");

static PyObject *pytun_tuntap_persist(PyObject *self, PyObject *args) {
    Py_RETURN_NONE;
}

PyDoc_STRVAR(pytun_tuntap_persist_doc, "persist(flag) -> None.\nMake the TUN/TAP persistent if flags is True else make it non-persistent.");
...
PyDoc_STRVAR(pytun_tuntap_mq_attach_doc,"mq_attach(flag) -> None.\nEnable the queue if flags is True else\ndisable the queue.");
#endif
...
#ifdef IFF_MULTI_QUEUE
    { "mq_attach",(PyCFunction) pytun_tuntap_mq_attach,METH_VARARGS, pytun_tuntap_mq_attach_doc },
#endif
    { NULL, NULL, 0, NULL }
};

PyDoc_STRVAR(pytun_tuntap_doc, "TunTapDevice(name='', flags=IFF_TUN, dev='/dev/net/tun') -> TUN/TAP device object.");
